...
def overview():
    st.text_area("개요",
                ...
                "- 데이터 분석 과정 \n \t 1. 현재 진행중인 이벤트에 대한 데이터 분석 \n \t \t ☛ 워드클라우드 분석: 댓글 내용에서 추출한 키워드들을 워드클라우드로 시각화하여 어떤 주요 주제들이 도드라지는지 확인. \n \t \t ☛ 감성 분석: 긍정적인 키워드와 부정적인 키워드의 빈도를 비교하여 이벤트에 대한 참여자들의 감정을 파악 \n \t"
                "2. 정기적으로 진행하는 이벤트(썬데이 메이플)에 대한 데이터 분석 \n \t \t ☛ 시계열 분석: 조회수, 추천수, 댓글 수의 시계열 데이터를 분석하여 이벤트의 성과 추이를 확인.특정 날짜에 조회수와 댓글 수가 증가하는 경향이 파악 \n \t \t ☛ 워드클라우드 분석: 조회수와 추천수의 합이 가장 큰 날과 작은 날에 대한 댓글 내용을 워드클라우드로 시각화하여 어떤 주제들이 큰 관심을 받았는지 또는 어떤 주제가 덜 주목받았는지 확인 \n \t"
                "3. 메이플스토리 직업별 게시글 데이터 분석 \n \t \t ☛ 막대그래프 분석: 최근 3개월 동안 각 직업의 조회수, 게시글 수, 댓글 수, 추천 수를 합산한 막대그래프를 통해 어떤 직업이 가장 활발하게 활동하고 있는지 확인 \n \t \t ☛ 토픽 모델링: 가장 인기 있는 직업 3가지와 가장 인기 없는 직업 3가지에 대해 게시글 제목을 활용하여 토픽 모델링을 진행함으로써 해당 직업들이 어떤 특정 주제에 대해 많은 관심을 가지고 있는지를 확인")
@st.cache_data
def layout1(df):
